...
| F06 | Agent 调度 | 创建任务、设置依赖、并发运行、暂停、恢复、取消并接收事件 | `apps/api/src/modules/agent-orchestration` | Nest API、状态机与 Prisma 运行快照已实现；真实 runner 通过注入接入 |
| F07 | 本机 Agent 接入 | 配对本机 Agent，发送心跳、上报事件、受控启动 Codex/Claude CLI | `apps/agent-connector`、`apps/api/src/modules/agent-connectors` | 配对、心跳、事件与本机进程适配器已实现；平台主动下发任务待补 |
...
下图中 Orchestrator 到本机 Connector 的远程任务下发仍是目标链路；当前已经具备可持久化调度 API 和本机 Provider 进程适配器，中间的任务领取、确认与断线恢复协议尚未接通。
...
当前落地原则：服务端不直接替学生机器执行任意 shell；本地命令由 Connector 在受限工作区内通过 `spawn(..., shell: false)` 启动。Codex、Claude 和显式白名单的自研可执行程序已经由统一 Provider adapter 承接，包含超时、取消、输出上限和敏感信息脱敏。平台到 Connector 的主动任务下发尚未接通，现阶段仍由本机 CLI 发起执行并回传结构化事件。
...
| Agent 调度 | `POST/GET /agent-orchestration/runs`、`POST /agent-orchestration/schedule`、运行暂停/恢复/取消接口 | 教师或后续调度控制台 |
...
| 提交 | `POST /submissions` | Day 面板、任务提交入口 |
...
1. **接通 Orchestrator 到 Connector 的远程 runner**：补齐任务领取/推送、执行确认、事件回执、断线恢复与幂等键，让服务端已持久化的 run 真正驱动学生机器上的 Provider。
